...
- Demonstrate whether snap-judgments are used and how effective they are for groups of people
- Demonstrate how snap-judgments may be used even for non-political objects and the consequences
- Add one potential mechanism for the puzzle of partisan sorting: attitude expression suggests it happens, but behaviorally it does not happen much.
- Test the boundary conditions for color being a useful partisan cue
...
As discussed thus far in the chapter, non-political objects are often associated with particular partisan leanings. For example, the types of cars that we drive are often used and rather effective cues to one's partisanship [@hetherington_prius_2018;@gebru_using_2017]. In the same spirit as the rest of this book, we do not need to necessarily distinguish between whether someone drives a hybrid sedan and a Ford pickup truck. Information giving insight into one's partisanship may even be as subtle as their color choice in car.

Recall the snap-judgment model as depicted in @fig-theory. To apply it to the scenario of partisan sorting, absent other information about the neighborhood, the prevalence of particular colors of the cars in a neighborhood may be information that we can use to make guesses about the partisan composition of a neighborhood. Just as the other chapters demonstrated, when politics is salient, the associations between the colors red and blue trigger snap-judgments that inform attitude expression as well as behavioral reactions, and these effects appear to exist even once we have been given more explicit, and thus more informative, information about political persuasions.
